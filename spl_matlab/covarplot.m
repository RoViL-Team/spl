...
%
%   Allows positive semi-definite covariance matrices, so one or more axes may
%   have zero radius, representing zero uncertainty in that direction.  This 
%   allows the same code to also plot 2D error ellipses and 1D confidence
%   intervals.
%
%   If the data is supplied in factored (E,V) form, then covariance matrices 
%   with one or more infinite eigenvalues are also supported.  Arrows are drawn
%   in the directions corresponding to zero certainty.
%
%   The plot is made in the current axes of the current figure, if any, else a 
%   new figure window is opened.  The return value h is an MxN array of handles
%   of Matlab hgtransform groups, each containing all generated graphics for one
%   data point.  (M and N are inferred from the inputs, see below).
%
...
%
%   Option 'cxx', 'cyy', 'czz', 'cxy', 'cyz', 'cxz' (default 0): data variances
%   and covariances.  May each either be present or absent and scalar or MxN.  
%   Mutually exclusive with 'E' and 'V'.
%
%   Option 'E' (default []): either [], Dx1xM, or DxDxM.  Ignored if empty,
%   otherwise mutually exclusive with 'cxx'--'cxz'.  If DxDxM then E(:,:,i) are
%   the data covariance matrices and 'V' cannot also be specified. If Dx1xM then
%   V(:,:,i)*diag(E(:,1,i))*V(:,:,i)' are the data covariance matrices and V 
%   must be DxDxM.  D must be in [1,3]; if D<3 then the given covariance 
%   matrices are extended to 3x3 by appending zeros to the right and bottom.  In
%   all these cases other per-datum inputs (e.g. cx, cy,cz) are expected to be 
%   Mx1 and the output data will be Mx1.
%
%   Option 'V' (default []): either [] or 3x3xM. Ignored if empty, otherwise see
%   'E'.
%
%   Options 'cx', 'cy', 'cz' (default 0): ellipsoid centers. May each either be
%   present or absent and scalar or MxN.
%
%   Option 'p' (default 0.95): confidence interval in [0,1), scales radii so
%   that the data is expected to lie within the ellipsoid with the given
%   probability. Scalar or MxN.
%
%   Option 'ns' (default 30): number of segments used to draw ellipses. Scalar.
%
...
%
%   Option 'pc' (default 'b'): color for center points. Either a colorspec or 
%   [r g b].
%
%   Option 'pt' (default 'o'): center point marker type.
%
%   Option 'da' (default 1): whether to draw RGB axes triad for each covariance
%   matrix.
%
...
%
%   Option 'fc' (default 'c'): ellipsoid face color, either a color name or
%   [r g b].
%
...
if (er==ec) % must eigendecompose
  ...
  for i=1:nd
    ...
    [v, d] = eig(e);
    if (det(v) < 0) v(1:3,3) = -v(1:3,3); end
