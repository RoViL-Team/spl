...
%
%   Option 'apts' (default 0): whether to consider only the points that are
%   above the patch.
%
...
newtonit = 15; newtonstop = 0.01; apts = 1;
...
if (iw) % transform to local frame
  ...
  if (apts) % only the points with positive z-value in the local frame
    x = xyz(xyz(:,3)>0,1); y = xyz(xyz(:,3)>0,2); z = xyz(xyz(:,3)>0,3);
    sz = size(x); nd = sz(1)*sz(2);
  else
...
% functions %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  if (~isempty(ll)) % select real root
    ...
    if (chkll) % check symbolic solution for ll
      ...
      if (any(abs(llr-sllr)>tol)); warning('llr(%s)~=sllr(%s)',llr,sllr); end
    end
