...
dfp0 = {'ppss',ppss,'ppas',ppas,'ssmax',ssmax,'dores','exact'}; %TBD: dores
...
rpshow = 2; asdbg = 3;
ssr = 0.05; ssk = 5000;
das0 = {'rpshow',rpshow,'ssr',ssr,'ssk',5000,'dbg',asdbg, 'maxr',1};
    ...
    if ((i>1)&&ischar(o{i-1}))
      ...
      if ((length(n)==2)&&(n(2)=='c')) % handle an auto color
        ...
        switch(n) % handle some other auto
          case 'ps'
            o{i} = aps; if (firstaps<0); firstaps = aps; end
...
for i=1:nd % for each dataset
  ...
  if (doms)
    ...
    pp{i} = [pp{i}, msp]; hpp{i} = [hpp{i}, arrayfun(@(p)(p.hp),msp,'uni',0)];
  end
